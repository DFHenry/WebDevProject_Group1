<?php 
    $pageTitle = "Delete Catering Item";
?>
...
$query = 'SELECT id, user_id FROM catering_order_items WHERE user_id = ? AND id = ? LIMIT 1';
...
{
    $query = "DELETE FROM catering_order_items WHERE id = ? AND order_id = ?";
    $stmt = $db->prepare($query);
    $stmt->bind_param('ii',
        $orderId,
        $userId
    ...
    if($stmt->execute() == false)
        ...
        {
            $_SESSION['success'] = "Catering item has been deleted successfully!"; // Set a success message in the session
...
<div class="mb-3">
  <p>Use the form below to delete your catering item. This action cannot be undone.</p>
</div>
<form action="delete-catering-items.php?id=<?= $order['id'] ?>" method="post">
